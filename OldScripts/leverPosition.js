...
function handleResponse(response) {
  ...
  switch (pktType) {
    case DYNO_RECV_MSG_READY:
      console.log('Device ready, lever position set successfully');
      return { type: 'ready' };
    default:
...
function setLeverPosition(port, position) {
  return new Promise((resolve, reject) => {
    if (position < 0 || position > 255) {
      reject(new Error('Invalid lever position. Must be between 0 and 255.'));
      return;
    }
    ...
    const packet = constructPacket(DYNO_SEND_MSG_LEVER_POSITION, dataBuffer);
    const encryptedPacket = encryptData(packet);

    const timeout = setTimeout(() => {
      reject(new Error('Timeout waiting for lever position set response'));
    ...
    function onData(data) {
      ...
      try {
        ...
        if (response) {
          ...
          reject(new Error('Invalid lever position set response'));
        }
      } catch (err) {
    ...
    port.write(encryptedPacket, (err) => {
      if (err) {
        ...
        reject(new Error(`Error sending lever position set request: ${err.message}`));
      } else {
        console.log('Lever position set request sent');
      }
    });
  });
}
...
async function main() {
  ...
  const leverPosition = 100;  // Change this to the desired lever position (0 = do not use, index starts at 1)
  ...
  try {
    ...
    console.log(`Setting lever position to ${leverPosition}...`);
    const result = await setLeverPosition(port, leverPosition);
    console.log('Lever position set result:', result);
  } catch (error) {
