...
const DYNO_SEND_MSG_RESET_LOAD_CELL = 0x0D;
...
function handleResponse(response) {
  ...
  switch (pktType) {
    case DYNO_RECV_MSG_READY:
      console.log('Device ready, load cell reset successful');
      return { type: 'ready' };
    default:
...
function resetLoadCell(port) {
  return new Promise((resolve, reject) => {
    const packet = constructPacket(DYNO_SEND_MSG_RESET_LOAD_CELL);
    const encryptedPacket = encryptData(packet);

    const timeout = setTimeout(() => {
      reject(new Error('Timeout waiting for load cell reset response'));
    ...
    function onData(data) {
      ...
      try {
        ...
        if (response) {
          ...
          reject(new Error('Invalid load cell reset response'));
        }
      } catch (err) {
    ...
    port.write(encryptedPacket, (err) => {
      if (err) {
        ...
        reject(new Error(`Error sending load cell reset request: ${err.message}`));
      } else {
        console.log('Load cell reset request sent');
      }
    });
  });
}
...
async function main() {
  ...
  try {
    ...
    console.log('Resetting load cell to default...');
    const result = await resetLoadCell(port);
    console.log('Load cell reset result:', result);
  } catch (error) {
