...
const DYNO_SEND_MSG_RECORD_FULLY_RETRACTED = 0x08;
...
function handleResponse(response) {
  ...
  switch (pktType) {
    case DYNO_RECV_MSG_READY:
      console.log('Device ready, fully retracted position recorded successfully');
      return { type: 'ready' };
    default:
...
function recordFullyRetracted(port) {
  return new Promise((resolve, reject) => {
    const packet = constructPacket(DYNO_SEND_MSG_RECORD_FULLY_RETRACTED);
    const encryptedPacket = encryptData(packet);

    const timeout = setTimeout(() => {
      reject(new Error('Timeout waiting for fully retracted position record response'));
    ...
    function onData(data) {
      ...
      try {
        ...
        if (response) {
          ...
          reject(new Error('Invalid fully retracted position record response'));
        }
      } catch (err) {
    ...
    port.write(encryptedPacket, (err) => {
      if (err) {
        ...
        reject(new Error(`Error sending fully retracted position record request: ${err.message}`));
      } else {
        console.log('Fully retracted position record request sent');
      }
    });
  });
}
...
async function main() {
  ...
  try {
    ...
    console.log('Recording position as fully retracted...');
    const result = await recordFullyRetracted(port);
    console.log('Fully retracted position record result:', result);
  } catch (error) {
