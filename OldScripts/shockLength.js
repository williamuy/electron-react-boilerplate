...
const DYNO_SEND_MSG_SHOCK_LENGTH = 0x09;
...
function handleResponse(response) {
  ...
  switch (pktType) {
    case DYNO_RECV_MSG_READY:
      console.log('Device ready, shock length set successfully');
      return { type: 'ready' };
    default:
...
function setShockLength(port, length) {
  return new Promise((resolve, reject) => {
    const dataBuffer = Buffer.alloc(8); // 8 bytes: 4 for float, 4 for padding
    dataBuffer.writeFloatLE(length, 0);
    
    const packet = constructPacket(DYNO_SEND_MSG_SHOCK_LENGTH, dataBuffer);
    const encryptedPacket = encryptData(packet);

    const timeout = setTimeout(() => {
      reject(new Error('Timeout waiting for shock length set response'));
    ...
    function onData(data) {
      ...
      try {
        ...
        if (response) {
          ...
          reject(new Error('Invalid shock length set response'));
        }
      } catch (err) {
    ...
    port.write(encryptedPacket, (err) => {
      if (err) {
        ...
        reject(new Error(`Error sending shock length set request: ${err.message}`));
      } else {
        console.log('Shock length set request sent');
      }
    });
  });
}
...
async function main() {
  ...
  const shockLength = 4; // Change this to the desired shock length in inches
  ...
  try {
    ...
    console.log(`Setting shock length to ${shockLength} inches...`);
    const result = await setShockLength(port, shockLength);
    console.log('Shock length set result:', result);
  } catch (error) {
