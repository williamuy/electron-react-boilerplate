...
import {
  sendPing, requestHardwareInfo, sendLeverPosition, checkConnection // Import sendPing from stm32Connection.ts
} from './STM32';

ipcMain.handle('check-connection', async (event, portName: string) => {
  try {
    const isConnected = await checkConnection(portName);
    return isConnected;
  } catch (error: any) {
...
ipcMain.handle('send-lever-position', async (event, portName: string, position: number) => {
  try {
    const result = await sendLeverPosition(portName, position);
    return result;
  } catch (error: any) {
    throw new Error(error.message);
  }
